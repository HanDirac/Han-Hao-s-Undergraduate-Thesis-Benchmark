...
@task "ad" nqubits=nqubit_list begin
    map(nqubit_list) do k
		#circ = fuse_gates(variational_circuit_1d(k, 10))
...
if !ispath("./sanity_check")
    mkpath("./sanity_check")
end

write("./sanity_check/sc_variational_circuit_test.json", JSON.json(benchmarks, 4; allownan=true))